...
class SceneRouter(object):
		...
		if not self._has_credentials():
			...
			while True:
				...
				if not self._has_credentials():
					...
					try_again = dialog.yesno(self.language(2006), self.language(2008), yeslabel=self.language(2009), nolabel=self.language(2010))
					if not try_again:
						return False
				else:
					return True
	...
	def connect(self):
		try:
			self.connection = self.cache.get('connection', default_producer = lambda: Connection(self._username, self._password))
